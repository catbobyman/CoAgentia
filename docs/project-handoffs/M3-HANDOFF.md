...
| 契约 schema 模型 | `packages/contracts` | TaskPlan/TaskHandoff/LoopContract Pydantic 模型 M1 已定（PRD §4.3 v1 schema）——契约 body 校验直接 model_validate |
...
- [ ] 1. Alembic `0003_m3` 落库：三表 + 约束；从零与增量升级双路绿（E1）
- [ ] 2. contracts 包 M3 登记完成：ENDPOINTS_M3 / MCP M3 工具组 / 契约请求响应模型，manifest 测试与 `pnpm gen` diff 为空；E 契约修订 v1.2 先行（E0）
- [ ] 3. 契约提交/修订链全绿：新 revision 置换、superseded 终态、kind↔schema 校验、request-draft 直投（E2）
- [ ] 4. T7 逐路径验收：l2 缺契约/缺字段拒（missing 指名）、齐备放行、l1 直通、Agent 经 MCP 同拒；M2 384+ 存量测试零回归（E3）
- [ ] 5. P5 契约卡真数据：TaskPlan/TaskHandoff 渲染、起草入口、HANDOFF_INCOMPLETE 缺失字段就地提示（B-M3-1）
- [ ] 6a. 块 a 守门：全量 pytest + vitest + 双侧 typecheck + ruff 全绿；三处交接文档同步（纪律 5）
