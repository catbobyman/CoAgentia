...
> 2026-07-20 起已由仓库根 [`plan.md`](../../plan.md) v1.0 取代执行权威；本文仅作摸底事实与旧方案参考。

v1.0 · 2026-07-19 · owner 指示「写一份完整详细的迁移计划，保证高效也保证质量」。
历史定位：当时对 [TS-MIGRATION-ROADMAP.md](TS-MIGRATION-ROADMAP.md) 的执行细化；其中“唯一计划事实源”和 §9 开工门均已失效，当前只可引用摸底事实。本文所有规模/行为数字来自 2026-07-19 两路实机摸底（server 结构全图 + 数据层/node 运行时实测探针），非估计值另有标注。
