...
| 文档 | 定位 | 维护规则 |
|---|---|---|
| [根 plan.md](../../plan.md) | **当前唯一迁移执行权威**（纯 TS 终态，2026-07-20 owner 批准，P0 进行中） | 阶段状态只在根计划进度账本与 P0 证据中推进 |
| [CURRENT-HANDOFF.md](CURRENT-HANDOFF.md) | 当前唯一有效的交接入口 | 每次阶段收口时更新 |
| [M8-HANDOFF.md](M8-HANDOFF.md) | M8 历史任务书（已收口，仅作事实参考） | 只读历史 |
| [M8-DEV-PLAN.md](M8-DEV-PLAN.md) | M8 历史执行计划（已收口） | 只读历史 |
...
> **M1–M8 已收口，均为历史阶段。** 迁移阶段的最新状态见 `CURRENT-HANDOFF.md`；阶段结论见 `PROJECT-RECORD.md`。任何历史“后续/接续”描述均不覆盖根 `plan.md`。
...
1. 新会话先读仓库根 `plan.md`，再读 `CURRENT-HANDOFF.md`；需要历史背景时再读 `PROJECT-RECORD.md`。
2. 迁移范围、依赖和进度以根 `plan.md` 为唯一执行权威；`CURRENT-HANDOFF.md` 保持日常运行状态与最新证据快照。
