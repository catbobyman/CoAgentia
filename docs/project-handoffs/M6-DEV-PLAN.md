...
| J12 | **实机 verify = PRD M6 出口** → `docs/verify/M6-EVIDENCE.md`（三场景+A1–A8+截图） | ✅ | 阶段 4 本提交 `818a483` | **48/48 ALL PASS**（Fable 亲跑）：真 uvicorn+真 websockets daemon-sim(真 git.py)+真 scratch 仓库，REST 扮演 Orch/工人全链走生产码。**S1 拆解全链**(A1/A3/A4:提案卡→删节点改标题确认→异步落地含汇总→两 worktree 交付→**--no-ff merge success**→check success)/**S2 冲突派回**(A6:冲突→failed→自动派回→解决→retry→合入)/**S3 修复循环**(A2:repairing 改好+三败升级 @人类)/**S4 A5 崩溃重放**(落地中 taskkill→重启续跑→10 节点无重复无缺失+已落地恰一条)/**S5 delta+O9**(Agent 403 rule=O9/部分接受 removed_ops/F9 base 过期 409+failed)/**S6 single_task**(A7:无汇总无 merge)/**S7 直落**(A8:auto(channel-policy))；真 git log 佐证双 --no-ff merge commit+冲突解决 retry；证据=M6-EVIDENCE.md + M6-VERIFY-results.json(48/48) + 2 截图(chat 叙事/canvas 落地 DAG) |
| — | /code-review high（块 b）→ M6 里程碑收口，任务书移 archive/ | ✅ | 阶段 4 收口本提交 | **8 维 Opus finder（workflow 编排）→ 逐条对抗核实（Opus verifier，默认证伪）→ Fable 终裁：13 findings（12 CONFIRMED/1 REFUTED）**。**已修 8 条**：CR-0（major）delta 同 revision 成功刷 base_hash（对称 rev+1，修复循环 base 自愈路径打通）/CR-3（major）模板 instantiate 补 O9 人类门（Agent 落地 DAG 越权闭合）/CR-4（major）kernel 5 处 `x in frozenset` 补 `_is_str` 守卫（unhashable 枚举值不再 500，与 TS 镜像双跑一致）/CR-5 golden +4 unhashable 判例锁定/CR-1（minor）delta remove_node TOCTOU→锁内重验（先删取锁后重读 task 状态，claim 逃逸闭合）/CR-2（minor）fail-closed 不再触发系统节点重扫（截断前缀图上 merge 空成功窗口闭合，只 LANDING_COMPLETED 重扫）/CR-6（minor）ProposalCard delta 卡指标读 operations 统计 +新增/−删除/CR-7·8（minor 效率）classify_submission 与 maybe_trigger_t1 短路免无谓联表。**+9 回归**（5 后端 test_delta CR-* + golden 4 双跑）；**登记不修**（观察级挂账，见 §5）：CR-9 _post_landed_message N+1 / CR-10 部分唯一索引谓词字面量双源 / CR-11 0009 downgrade agent_skills FK。1 REFUTED（对抗核实证伪）。守门 947/354/pyright0/ruff/gen/build 全绿 |
| — | **M6 里程碑收口** → PROJECT-RECORD 终态同步、任务书移 archive/ | ✅ | 收口本提交 | M6a+M6b 全收口 = PRD M6 出口达成 |
...
| **J12 实机 verify** | 943 / 4（脚本外，48/48 ALL PASS） | — | 真 uvicorn+真 daemon+真 git，A1–A8 全勾销；M6-EVIDENCE.md + M6-VERIFY-results.json + 2 截图 | ✅ `818a483` |
| **阶段 4 /code-review high 收口** | 947 passed / 4 skipped（+4） | 354 passed（+5） | pyright 0 + 双 tsc / ruff 全绿 / gen 确定（golden +4 unhashable 判例双跑锁定，54 例）/ build 绿；**8 维 Opus finder→对抗核实→Fable 终裁 13 findings（12 CONFIRMED/1 REFUTED），修 8**（3 major：base_hash 对称/instantiate O9/kernel unhashable；4 minor：TOCTOU/fail-closed 不重扫/ProposalCard delta 指标/短路效率；1 golden）；3 观察挂账见 §5 | ✅ 收口本提交（见 HEAD） |
