...
| 契约 | 版本 | 本次修订内容 | 状态 |
| --- | --- | --- | --- |
| A 实体表 | v1.0.6 → **v1.0.7** | `tasks` 增 `project_id`/`writes_code` 两列（W2 分级的任务级落点，**首例既有表加列迁移**随 0008）；`projects` 补 `computer_id`（D §12 #1 顺延件收回）；`TaskHandoffBody` 扩可选 `review_verdict`（FR-7.5，零迁移）；`TaskDetail` 扩读面派生 `worktree`（Public 放宽第 6 例）。核对确认：canvas_nodes 系统列 **0003 已建全**、channels.decomp_mode/decomp_node_limit **M1 已建**、proposals/worktrees/channel_projects 形状冻结无修订。**M6 批次 = 0008（M6a 三表+tasks 两列）+ 0009（M6b proposals）** | ✅ 已落笔 |
| B REST | v1.3 → **v1.4** | 新 **§12 编排与交付规范条文**（提案生命周期接缝/校验内核单源=纪律 8 扩展/修复循环直投/确认 CAS 与 delta 部分接受/落地事务含 merge 节点自动追加与 fail-closed 复核义务/worktree 生命周期与**工作目录消息注入**/Diff 卡/合并 **--no-ff** 与冲突派回/系统节点执行/评审结论枚举/模板治理/Project 治理）；§4.12 +PATCH/DELETE templates（挂账收回）；§4.9 create_node 扩 writes_code/project_id；错误码 +SYSTEM_NODE_NOT_RETRYABLE/+TEMPLATE_BUILTIN_IMMUTABLE/+PROJECT_IN_USE（25→28）；§8 +repo_path 多机校验开放问题 | ✅ 已落笔 |
...
> **纪律 1 中间态**：契约文档修订已先行完成；contracts 包 manifest 同步 = J0 模块（开工第一步之一），任何业务实现之前完成。
...
| J1 | **M6a** | 0008 迁移 | Alembic `0008_m6a`：projects + channel_projects + worktrees 三张（A §4.9，含 computer_id）+ **tasks 加 project_id/writes_code 两列**（首例既有表加列——SQLite ADD COLUMN，批内幂等纪律同 0004 if_not_exists 先例）；models ORM + `M6A_TABLES`；索引最小集（worktrees.task_id UNIQUE 即查询键；channel_projects 复合 PK） | 从零 `upgrade head` 与 M5 库增量升级双路绿；表结构对照测试扩展；加列后既有 tasks 全量测试零回归 |
...
| 14 | 迁移两批：0008（M6a 三表+tasks 两列）/ 0009（M6b proposals）——每块一批,块内一次建齐 | A v1.0.7 ⑥ |
...
- [ ] 1. J0 契约登记：ENDPOINTS_M6 / Project·Worktree·Diff·Proposal 模型 / TaskPublic·TaskDetail·TaskHandoffBody·CanvasNodeCreate 扩字段 / 错误码 28 / ws 预留族核对 / D 新帧 / mock 形状，catalog 与 `pnpm gen` 两跑一致
- [ ] 2. Alembic `0008_m6a`：三表+tasks 两列，从零与增量双路绿；对照测试；既有 tasks 面零回归
