...
## 16. M8 实用化加固与编排质量（块 M8a + 块 M8b 收口 2026-07-14；块 M8c 未开工）
...
| MVP 全里程碑 M1–M7 完成，无后续 PRD 里程碑（M8+ 未立项） | **M8 已立项（[M8-HANDOFF.md](M8-HANDOFF.md)，MVP 后实用化加固+编排质量+单机产品化）；块 M8a+M8b 已收口（§16），接续 = 块 M8c** |

## 当前接续任务

1. **块 M8b「O8 编排质量线」收口（2026-07-14）= 接续块 M8c**：M8 已立项（[M8-HANDOFF.md](M8-HANDOFF.md)），块 M8a 加固批（`ab84406`）+ 块 M8b（后端 L7 `cb5d00b`·L8 `7d4c910`+`e68abcd`·L9 `872762a` + 前端 B-M8-2 `a51637c`）已收口，见 §16。守门 **1117/4 skipped + web vitest 502 + pyright 0 + ruff 净 + gen 确定 + build 绿**。契约 A **v1.0.12**/B **v1.5.2**/C v1.0/D v1.0.5/E v1.5/E2 v1.0.1（错误码仍 29）。**接续 = 块 M8c**（B-M8-3 产品化外壳 + L10/L11 打招呼[owner 可否决]/L12 编排体验收官+教程/**L13 m8_verify 收口 = O8 全链真机演示 §9b #11 + 浏览器 E2E**）；恢复入口先读 [CURRENT-HANDOFF.md](CURRENT-HANDOFF.md) §4 + [M8-DEV-PLAN.md](M8-DEV-PLAN.md) §1「块 M8c」。历史 MVP M1–M7 收口见下 #2/§15。
