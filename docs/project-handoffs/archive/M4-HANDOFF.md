...
| **M4b code-review（#8/#9）** | `_post_system_message`（hub「三处共用」+ guard 第 4 份 + channels 瘦变体）/ `_channel_human_members`（hub/guard 两份）跨模块重复——确认 DRY 债非 bug，消除需抽中立共享模块并触及承重 reminder/silence 路径，收口期 ROI 低 | 顺手小批（tests 守回归） |
