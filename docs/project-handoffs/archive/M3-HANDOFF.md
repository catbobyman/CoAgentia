...
| 出处 | 问题 | 归属 |
| --- | --- | --- |
| M2 挂账/二轮 review | ~~消息流附件卡数据源 = channelFiles 首页 ≤50~~ **已修（2026-07-10 挂账批1）**：消息级关联，契约 A v1.0.4 `MessagePublic.files` 读面派生 + 0004 files 索引；证据 [B1-ATTACH-EVIDENCE.md](../verify/B1-ATTACH-EVIDENCE.md) | ~~M3a 尾部顺手件~~ 已收 |
| M2 挂账 | ~~id 游标分页 after 行离开结果集重发首页 + activity/files 无 SQL LIMIT 全量材料化~~ **已修（2026-07-10 挂账批2）**：`keyset_page`（(created_at,id) 行值锚点 + LIMIT 下推）四端点统一；messages before 修成紧邻回翻；证据 [B2-KEYSET-EVIDENCE.md](../verify/B2-KEYSET-EVIDENCE.md) | ~~独立小批~~ 已收 |
| 二轮 review 挂账 | ~~ActivityScreen 三档缓存双请求~~ **已修（同批2）**：'all' 单拉 + tab 客户端过滤，wsBridge 删多档 patch；Mentions 徽标(未读)与列表(含已 done)口径差异为**有意设计**（列表灰显=历史），保持不改 | ~~简化归 keyset 批~~ 已收；口径差异不改 |
...
| M1 遗留 | ~~pyright 109 既有错误~~ **已清零（2026-07-10 挂账批3，实清 133 含 M2/M3a 新增）**：`models.tbl()` Table 窄化（80×FromClause DML）+ `models.row_dict()` 非空窄化 + 零星标注修正；pyright 已并入根级 `pnpm typecheck` 守门 | ~~独立批处理~~ 已收 |
...
- [x] 7. 画布结构端点全绿：建节点=建 L2 任务+锚点消息（第三途径）、删节点保任务、无环拒 GRAPH_CYCLE、layout 不 bump 基线、baseline_advanced 广播（E4）——test_canvas.py + 实机 17/17
- [x] 8. blocked 推导 + 投递 gating：上游未 done 不唤醒、done 解锁、force-start 仅人类 + 双留痕；状态写不受限（R4/R7 回归）（E5）——test_gating.py/test_force_start.py + daemon-sim 实机；review #1 修（gating 作用于投递批含水位截断，不消费 gated 消息）
- [x] 9. P2 画布：React Flow 可视化 + 实时着色 + 拖拽编辑（建节点/连断边/重指派）+ 成环预判 + 节点深链双向（B-M3-2）——vitest + 浏览器截图（拖拽中不回弹 review #7 修）
- [x] 10. 升格流：新建节点弹层补契约（Agent 起草/手填）+ PATCH level l1→l2；force-start UI 二次确认（B-M3-3）——vitest + 浏览器 ForceStartModal 截图
- [x] 11. **工程三角全流程真机实证**（PRD M3 出口）：六节点 DAG 建图 → blocked 标注与不唤醒 → 逐节点推进含 T7 门 → force-start 留痕 → 全程 WS 无刷新；截图归档 `docs/verify/`（E6）——真 uvicorn + 真 websockets daemon-sim 17/17 + 6 截图（[M3B-EVIDENCE.md](../verify/M3B-EVIDENCE.md)）
- [x] 12. 浮动件收口：FTS trigram 中文命中实测 + 结论回写 A §10.4（3 字符地板：≥3 字 MATCH + <3 字 LIKE 兜底）——0005 迁移 + test_fts_trigram.py
- [x] 13. 终收口守门：全量测试绿（后端 483+3 skipped、web vitest 76、pyright 0、ruff、gen 确定、双侧 build）；M3 阶段结论写入 [PROJECT-RECORD.md](../project-handoffs/PROJECT-RECORD.md)，本任务书移入 archive/（README 维护约定 3）
