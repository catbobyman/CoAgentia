...
| 一句话状态 | **M1–M5 全收口；M6a 全收口（实现+实机 verify 20/20+code-review 10 findings 全修）**。**M6b 进行中（COLLAB-MODEL v2 Fable 单窗编排）**：波 1（J7 内核 ✅ + J11 骨架 ◐，`95d190c`）与波 2（J8 提案域 ✅）已收口提交；契约 A **v1.0.10** 已落 0009；接续 = 阶段 2 J9（Fable 硬关口）。 |
...
| 测试基线 | 后端 **895 passed / 4 skipped**（`uv run pytest -q`，M6b 波 2 起）· web vitest **261** · pyright **0**（并入 `pnpm typecheck`）· ruff 干净（波 1 顺修 M6a 遗留 scratchpad 4 条 lint）· `pnpm gen` 确定 · web build 绿 |
