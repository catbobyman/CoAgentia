...
| 一句话状态 | **M1–M5 全收口；M6a 全收口（实现+实机 verify 20/20+code-review 10 findings 全修）**。M6b 未开工（接续入口 = COLLAB-MODEL §4a 的 Opus kickoff）。 |
...
| 测试基线 | 后端 **827 passed / 4 skipped**（`uv run pytest -q`）· web vitest **195** · pyright **0**（并入 `pnpm typecheck`）· ruff 干净 · `pnpm gen` 确定 · web build 绿 |
