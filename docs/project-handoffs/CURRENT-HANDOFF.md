# CoAgentia 当前交接（**MVP M1–M7 完成；M8 进行中——块 M8a「加固批」已收口**）
...
| 定位 | **当前唯一有效的交接入口**（README 约定 1/2）：新会话先读本文；历史背景读 [PROJECT-RECORD.md](PROJECT-RECORD.md)（§13=M6a、§14=M6b、**§15=M7**）。**M8 进行中**：任务书 = [M8-HANDOFF.md](M8-HANDOFF.md)（M8a 加固批 ✅ / M8b O8 编排质量线 / M8c 单机外壳+体验收官），执行计划与进度 = [M8-DEV-PLAN.md](M8-DEV-PLAN.md)。**接续 = 块 M8b（L7 W9 内核先行——迁移 0012 已随 L0 落，L7 只剩双档 satisfied+golden partial+landing 默认 partial+patch_node 改档；实现权威 [Orchestrator汇总设计.md](../../../orchestrator_docs/Orchestrator汇总设计.md) v1.0）**。多租户/多机/多副本挂账（[M7-RESERVATION-AUDIT.md](../M7-RESERVATION-AUDIT.md) R-1~R-9/R-11/R-12/R-15；~~R-10/R-13/R-14~~ 已随 M8a 收）维持 M9+ 未立项。 |
...
| 守门终态（M8a 收口） | 后端 **1089 passed / 4 skipped**（M8a 起点 1075）· web vitest **463**（M8a 起点 444）· pyright **0**（并入 `pnpm typecheck` + 双 tsc）· ruff 干净 · `pnpm gen` 确定（M8a 无 gen 漂移，L0 后逐字节稳定）· web build 绿 · **工作树干净**。块 M8a 实机 verify 归 M8c L13（O8 全链需 M8b 落地后一并演示）——本块以单测 + 探针（K1 空成功回归 / L4a 写错不撕连接·阻塞写不阻塞读循环 / R-10 崩溃时序）守门。 |
...
| 契约版本 | A **v1.0.12**（M8a L0：summary_runs 表 + canvas_nodes.upstream_policy 列，35 表，迁移 0012）· B **v1.5.1**（M8a L0：NodeCreate.upstream_node_ids + replan 403 rule=O8 + §2 补登 O9/O8；**错误码零新增仍 29**）· C **v1.0** · D **v1.0.5** · E **v1.5** · E2 **v1.0.1**（C/D/E/E2 M8a 零修订核对）。contracts 新增 `UpstreamPolicy` 枚举 / `SummaryRun` 模型 / `RULE_CODES` 补 O8·O9。**Agent 工具仍 16**（M8a 零新增；M7 的 `trigger_deploy` 为最后一件）。O8 执行期消费（摘要/护栏/W9 内核）归 M8b。事实源 = `D:\Project4work\Agenthub_7_8\engineering_docs\` 六契约 + `docx_agenthub\CoAgentia-PRD.md`（FR-11/FR-12/W4/W7/R8）+ 交互说明 §12 + `orchestrator_docs\Orchestrator任务拆解设计.md`（拆解实现级权威） |
...
```
uv run pytest -q                    # 当前 1089 passed / 4 skipped（M8a 起点 1075/4），只增不减
pnpm -F @coagentia/web test         # 当前 vitest 463（M8a 起点 444），只增不减
