...
| L12 体验收官+教程 | ✅ 完成 | （c-2 待提交） | **C1/C3 教程收官**：`使用教程/README.md` 加 §13「M8 新增」（13.1 新建频道 / 13.2 入职问候开关位置+默认关+一次性 / 13.3 O8 汇总横幅轮数·阻断-恢复+partial badge+改档 / 13.4 质量回流）+ §10 建 Agent 通用入口三处 + 速查表 4 行 + 头部 M8 提要。**C2 delta 审查面 UX 复盘结论**：B-M8-1 ③ 预览遮挡（.panel z-index 41 越 preview-deck）修后，delta 面板绿红高亮/逐 op 剔除实时重验/base 横幅在预览开启态可点可交互（Playwright 已证，M8a）——delta 审查 UX 无残留遮挡问题，结论 = 保持现形，无需再改（复盘归档于此）。**C1 多节点真机演示**：真机拆解流程截图既存 `使用教程/`（CR-M8 E2E 产出，19 图含真实拆解）；O8 多轮空转红例按防返工锚点 7 用单测不烧真 CLI |
| L13 m8_verify 收口 | ✅ 完成 | （c-3 待提交） | **`scratchpad/m8_verify.py` 10/10 ALL PASS**（真 uvicorn + 真 daemon-sim + 隔离库）：P0 连上 / S1 新建频道 / S2 建 Agent / L11-0..4 入职问候（默认关·开→线级收 SYSTEM inject 一条·标记一条·重启不重复）/ L1-1 原子建边非空成功 / L1-2 悬空 422。**浏览器可视 E2E**（同源 build SPA + --keep）：Members「创建 Agent」+ 侧栏「新建频道」弹窗 DOM 核实、shell 建的 m8-shell/ShellBot 反流 UI、零 console 错误（截图工具本机超时，get_page_text/javascript_tool 核实，同 B-M8-2 体例）。证据 = [M8-EVIDENCE.md](../verify/M8-EVIDENCE.md) + [M8-VERIFY-results.json](../verify/M8-VERIFY-results.json)。O8 护栏正确性归单元套（1122 passing，防返工锚点 7） |
| /code-review 终收口 | ✅ 完成 | （c-4 待提交） | **high 8 角 finder → 5 findings，全低危，0 修**（终裁 = 均低危不阻断收口，登记挂账/观察项）：**F1 CONFIRMED** _maybe_onboarding_greet 读标记→插标记非原子（diagnostic_events 无 (agent,type) 唯一约束）→ 并发双 START 双问候（TOCTOU，违 CAS 纪律但效应良性=重复问候，单用户双击才触发）→ **挂账**（修 = 0013 偏索引 + savepoint 捕 IntegrityError，效应良性故不为单用户双击上迁移）；**F2 CONFIRMED** server_default 1→0 不改既有库行——既有 dev 工作区（旧 seed=true）保留 greeting-on → 违裁决 #9「默认关」于既有数据 → **挂账/文档**（新装默认关已兑现；既有工作区留原值，设置里可关；不强迁数据免与 owner「测问候」目标冲突）；**F3** 无标记 Agent 下次 START 均问候（非仅新建）——良性，标记保一次性（观察项）；**F4** setup 步骤 002 建 Agent 不推进 setup_state（既有：001/003 同不推进，setup 非门 = / 直达主壳，观察项）；**F5 PLAUSIBLE** ChannelList onCreated 选中尚未进快照的频道——自愈 UX 闪，与既有 useCreateDm 同型（不改）。守门全绿不变 |
