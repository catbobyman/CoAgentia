...
> **总结**：M7b 未在多租户覆盖面留新债（deployments 表已直挂 workspace_id）。新登记的残余窗口（R-10~R-14）均为 display-only、契约授权的设计固有窗口、或 MVP 明确接受的 win32 硬崩溃泄漏——**无一影响状态机正确性**（部署状态推进全程条件 UPDATE CAS + 副作用不可重放 fail-closed 双保险）。三条信任基座（单工作区/单机/单进程）的所有依赖点已逐一定位，M8+ 消费路径清晰。**本文档零代码变更，交 owner 过目。**
