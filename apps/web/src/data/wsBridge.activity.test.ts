...
describe('wsBridge activity.created', () => {
  ...
  it("只维护 'all' 单档(挂账批2 简化:tab 过滤归客户端),不为偏档建缓存", () => {
    const qc = new QueryClient();
    ...
    expect(qc.getQueryData<ActivityItemPublic[]>(qk.activity('mentions'))).toBeUndefined();
  });
});

describe('wsBridge activity.done', () => {
  it("在 'all' 档里把该 item 的 done_at 置为事件时间戳", () => {
    const qc = new QueryClient();
    qc.setQueryData<ActivityItemPublic[]>(qk.activity('all'), [
      activityItem('act_1'),
      activityItem('act_2'),
