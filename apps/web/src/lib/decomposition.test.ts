...
// ---- stripControl（UI 剥离散文,B-M6-2 提案卡）:非校验判定、不进 golden——只保证与
// extractControlBlocks 同一套标签定界语义（残缺开标签不当散文渲染）。
describe('stripControl 提案正文剥离 <control> 块', () => {
  it('剥离唯一控制块,保留前后散文', () => {
    const text = '拆解思路如下。\n\n<control>{"version":"v1"}</control>\n\n请审阅。';
    expect(stripControl(text)).toBe('拆解思路如下。\n\n\n\n请审阅。');
    expect(stripControl(text)).not.toContain('version');
  });
  it('多块全部剥离（与 parseControl 的多块报错独立——渲染面不放行任何机读体）', () => {
    const text = 'a<control>{"x":1}</control>b<control>{"y":2}</control>c';
    expect(stripControl(text)).toBe('abc');
  });
  it('无控制块原样返回（trim）', () => {
    expect(stripControl('  纯散文  ')).toBe('纯散文');
  });
  it('残缺开标签:其后不当散文渲染（防机读体泄漏进消息流）', () => {
    expect(stripControl('前文<control>{"未闭合":1}')).toBe('前文');
  });
});
