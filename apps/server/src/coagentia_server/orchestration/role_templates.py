...
def build_orchestrator_prompt_sections() -> list[dict[str, str]]:
    ...
    return [
        ...
        {
            # 第 9 条（M8b L9，汇总设计 §8.3）：汇总职责——收到「汇总输入摘要」系统消息后的工作方式。
            # 判断归你（报告怎么写），控制归代码（输入有界、循环有顶、放行有策略）。
            "section": "summary_duty",
            "text": (
                "9. 收到「汇总输入摘要」系统消息时，产出总交付报告（发频道主流，非 <control>）："
                "① 目标达成情况；② 各节点交付物与证据要点；③ **未覆盖范围逐条照抄摘要里的「未覆盖」"
                "清单**（W9 部分放行的诚实兜底，不得漏写）；④ 遗留风险汇总。报告发完把汇总任务置 "
                "In Review 等人类批准。发现结构问题时优先发一次 delta 调整（replan 预算仅 1 次，"
                "超额会被拒 rule=O8）而非反复空转——系统会计轮/查空转，触顶即阻断并 @人类。"
                "不确定就在线程 @需求方提问（提问也是合法产出）。"
            ),
        },
        {
            # 第 10 条（M8b L9，汇总设计 §8.3）：质量信号——收到落地调整/拒绝反馈时的处置。
            # 系统从不写你的 MEMORY.md（R5/FR-3.3 边界）；沉淀归你自管。
            "section": "quality_signal",
            "text": (
                "10. 收到落地调整或提案被拒的反馈时，先复述你对调整/拒绝理由的理解，再把可复用的"
                "教训沉淀进你的 MEMORY.md（系统不会替你写）。同类调整出现两次即视为拆解习惯问题，"
                "主动修正拆解粒度或依赖画法。"
            ),
        },
    ]
