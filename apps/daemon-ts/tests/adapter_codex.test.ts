...
// ========================================================= LineReader（校准条款 2 行读底座）

describe('LineReader（校准条款 2；无 py 对等：py 用 asyncio StreamReader）', () => {
  it('多 chunk 单行重组逐字节精确（UTF-8 多字节码点跨 chunk 边界）', async () => {
    const r = new LineReader();
    const line = Buffer.from('前缀-中文𠀀混合-' + 'x'.repeat(1000) + '\n', 'utf-8');
    // 7 字节步长切 chunk：必然在多字节码点中间切开（字节切行不 toString，cal1 规则 4）
    for (let i = 0; i < line.length; i += 7) {
      r.push(line.subarray(i, Math.min(i + 7, line.length)));
    }
    expect((await r.readline()).equals(line)).toBe(true);
  });

  it('一 chunk 多行 + 跨 chunk 续行 + CRLF 容忍（\\r 保留在行内）', async () => {
    const r = new LineReader();
    r.push(Buffer.from('a\r\nb\nc', 'utf-8'));
    r.push(Buffer.from('d\n', 'utf-8'));
    expect((await r.readline()).toString('utf-8')).toBe('a\r\n'); // 只按 \n 切；\r 由消费端 trim
    expect((await r.readline()).toString('utf-8')).toBe('b\n');
    expect((await r.readline()).toString('utf-8')).toBe('cd\n');
  });

  it('未终止行超限强制切出为一行继续读（不丢弃——区别于 claude 丢帧语义）+ EOF 残段/后续恒空', async () => {
    const r = new LineReader(8);
    r.push(Buffer.from('0123456789', 'utf-8')); // 10 > 8：强制切出，不崩不丢
    expect((await r.readline()).toString('utf-8')).toBe('0123456789');
    // 完整行（带 \n 到齐）不受上限检查——上限只管未终止累积（新旧实现同语义，锁死）
    r.push(Buffer.from('abcdefghij\nok\n', 'utf-8'));
    expect((await r.readline()).toString('utf-8')).toBe('abcdefghij\n');
    expect((await r.readline()).toString('utf-8')).toBe('ok\n');
    r.push(Buffer.from('tail', 'utf-8'));
    r.end(); // EOF 残段作末行
    expect((await r.readline()).toString('utf-8')).toBe('tail');
    expect((await r.readline()).length).toBe(0); // EOF 后恒空 Buffer（py b"" 对等）
    expect((await r.readline()).length).toBe(0);
  });

  it('跨多 chunk 累积超限：恰一次强制切出，其后行序照常', async () => {
    const r = new LineReader(8);
    r.push(Buffer.from('01234', 'utf-8'));
    r.push(Buffer.from('56789', 'utf-8')); // 累积 10 > 8 → 全部累积段一次切出
    expect((await r.readline()).toString('utf-8')).toBe('0123456789');
    r.push(Buffer.from('ok\n', 'utf-8'));
    expect((await r.readline()).toString('utf-8')).toBe('ok\n');
  });
});
