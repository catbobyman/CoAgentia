...
#
# 三种 schema 现在建齐（TaskPlan/TaskHandoff 供 M3 契约端点消费；LoopContract 的生成消费归 M4，
# 但表形状/模型 M3 就位）。`ContractCreate.body` 保持 JsonValue，按 kind 的二次 model_validate
# 在 server 侧执行（E2）——此处只登记契约形状，不在 rest.py 里做 kind↔模型分派。
