...
class TemplatePatch(ContractModel):
...
class DecomposeRequest(ContractModel):
    ...
    def exactly_one_source(self) -> "DecomposeRequest":
        if (self.text is None) == (self.task_id is None):
            raise ValueError("exactly one of text or task_id is required")
        return self


class ProposalConfirmExpected(ContractModel):
    proposal_hash: Sha256Hex
    baseline_version: int
    baseline_hash: Sha256Hex


class ProposalConfirm(ContractModel):
    expected: ProposalConfirmExpected
    adjustments: list[JsonValue] = []
    removed_ops: list[int] = []


class ProposalReject(ContractModel):
    reason: str | None = None


class ProposalConfirmResult(ContractModel):
    batch: LandingBatchPublic
...
# ---------------------------------------------------- M6 端点清单
# 编排 4 + Project 7 + retry 1 + 模板治理 2。

ENDPOINTS_M6: tuple[tuple[str, str], ...] = (
    # §4.10 编排
    ("POST", "/channels/{channel_id}/decompose"),
