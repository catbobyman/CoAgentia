...
class ErrorCode(StrEnum):
    ...
    COMPUTER_HAS_PROJECTS = "COMPUTER_HAS_PROJECTS"  # 409（M6a；Project 引用门）
...
class TaskDetail(ContractModel):
    ...
    usage: TaskUsage
    worktree: WorktreePublic | None = None


class TaskMergeAccepted(ContractModel):
    """POST /tasks/{task_id}/merge 202 响应（DEDAG B v1.6 §14）。
...
#
# 三种 schema 建齐：TaskPlan（`AcceptanceCriterion`/`TaskPlanBody` 已下沉 entities，此处从
# entities re-export 供契约端点/CONTRACT_BODY_MODELS 用）；TaskHandoff/LoopContract 仅请求侧
# 消费，留本模块。`ContractCreate.body` 保持 JsonValue，按
...
# ---- 4.11 Project（M6；4.10 编排画布域 DEDAG 退役——拆解/提案/落地模型随域移除）
...
class ProjectCreate(ContractModel):
    ...
    worktree_keep_days: int | None = None
    preview_idle_min: int | None = None


class ProjectPatch(ContractModel):
    name: str | None = None
    repo_path: str | None = None
    computer_id: Ulid | None = None
    dev_command: str | None = None
    deploy_command: str | None = None
    worktree_keep_days: int | None = None
    preview_idle_min: int | None = None
...
class ProjectBind(ContractModel):
    project_id: Ulid
...
# ---------------------------------------------------- M3 端点清单（§4.7 契约/force-start）
# 画布组（§4.9）DEDAG v1.6 退役：7 端点整组移除（表冻结，历史见契约 A §10）。
...
# ---------------------------------------------------- M5 端点清单（§4.5 通知设置二）
# 模板组（§4.12）DEDAG v1.6 退役：3 端点 + 治理 2（原 M6）整组移除。
...
# ---------------------------------------------------- M6 端点清单
# Project 7（编排 4 + retry 1 + 模板治理 2 随 DEDAG v1.6 退役移除）。

ENDPOINTS_M6: tuple[tuple[str, str], ...] = (
    # §4.11 Project 与交付链路（M6 行）
    ("GET", "/projects"),
    ("POST", "/projects"),
...
# ------------------------------------------- DEDAG 端点清单（B v1.6 §14 任务级 merge）
# 画布/提案/模板域退役后交付合并的唯一 REST 通道：人类 UI 按钮与 Orchestrator trigger_merge
# 工具同道（1:1 映射，E v1.7）。202 语义见 TaskMergeAccepted。

ENDPOINTS_DEDAG: tuple[tuple[str, str], ...] = (
